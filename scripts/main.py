...
# functions
def CalculatePixelDistance(target: tuple[int, int], crosshair: tuple[int, int]) -> tuple[int, int]:
...
while cap.isOpened():
    ...
    if key == ord('q'):
        ...
        cv2.putText(frame,
                    ...
                    color=color_dict["yellow"],
                    thickness=1)
        bbox = cv2.selectROI(windowName="Webcam Feed", img=frame, showCrosshair=True, fromCenter=False)
    ...
    if bbox != (0, 0, 0, 0):
        ...
        if ok:
            ...
            cv2.rectangle(frame, (bbox[0], bbox[1]), (bbox[0] + bbox[2], bbox[1] + bbox[3]), color_dict["red"], 3)
            cv2.line(frame, (mid_x, bbox[1] + bbox[3] // 3), (mid_x, bbox[1] + bbox[3] - bbox[3] // 3),
                     color_dict["blue"], 2)  # vertical
            cv2.line(frame, (bbox[0] + bbox[2] // 3, mid_y), (bbox[0] + bbox[2] - bbox[2] // 3, mid_y),
                     color_dict["blue"], 2)  # horizontal
    ...
    cv2.line(frame, (mid_frame[0], FRAME_HEIGHT // 3), (mid_frame[0], FRAME_HEIGHT - FRAME_HEIGHT // 3),
             color_dict["green"], 2)  # vertical
    cv2.line(frame, (FRAME_WIDTH // 3, mid_frame[1]), (FRAME_WIDTH - FRAME_WIDTH // 3, mid_frame[1]),
             color_dict["green"], 2)  # horizontal
